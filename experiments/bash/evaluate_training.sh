...
#
# @description:
# Bash script for evaluating the training.
#
# Usage: bash evaluate_training.sh
...
INPUT_PATH="${CHECKPOINT_BASE_PATH}/train"
OUTPUT_PATH="${RESULT_BASE_PATH}/train"
...
portion_augmented_samples=(0.33)
batch_size=64
      ...
      for seed in "${seeds[@]}"; do
        for portion in "${portion_augmented_samples[@]}"; do
          for augmentation in "${augmentations[@]}"; do
            accelerate launch $ACCELERATE_ARGS ./evaluate_training.py \
              --dataset $dataset \
              --data_path $DATA_PATH \
